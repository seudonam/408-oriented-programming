...
struct Trie {
  ...
  void erase(std::string_view key) { root = erase(root, key, 0); }
  Node *erase(Node *x, std::string_view key, int d) {
    if (x == nullptr)
      return nullptr;
    if (std::cmp_equal(d, key.length())) {
      if (x->val == true)
...
int main() {
  ...
  assert(retrieval.contains("erase"));

  retrieval.erase("size");
  retrieval.erase("empty");
  retrieval.erase("contains");
  retrieval.erase("insert");
  retrieval.erase("erase");
  ...
  for (auto &e : retrieval.keysWithPrefix(""))
    std::print("{}\n", e);
